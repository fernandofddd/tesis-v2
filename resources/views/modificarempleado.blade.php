...
<div class="container">
    ...
    <form action="{{route('guardarcambios')}}" method="POST" enctype="multipart/form-data">
        {{csrf_field()}} <!--middle word que permite evitar ataques de cross side y que terceros envien peticiones al sistema-->
        <div class="well">
            <div class="form-group">
                <label for="dni">Clave estudiante:
                    @if($errors->first('ide'))
                ...
                <!--id para nombre java y name para laravel -->
            </div>
            <div class="row">
            ...
            <div class="form-group">
                <label for="id">Foto perfil:
                    <img src="{{asset('archivos/'.$consulta->img)}}" height="50" width="50">
                    @if($errors->first('img'))
            ...
            <div class="row">
                <div class="col-xs-6 col-md-6"><input type="submit" value="Guardar" class="btn btn-danger btn-block btn-lg" tabindex="7" title="Guardar datos ingresados"></div>
            </div>
    </form>
    @stop